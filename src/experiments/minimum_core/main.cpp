...
// [css2025_win32] The minimum core.dll
...
sys.stderr = sys.stdout
    ...
    PyObject* stdout_redirector = PyObject_GetAttrString(sys, "stdout");
    PyObject* buffer = PyObject_GetAttrString(stdout_redirector, "buffer");

    PyRun_SimpleString("import traceback");
    //PyRun_SimpleString("import socket");
    import("socket");

    // flush stdout from python
    if (PyList_Check(buffer)) {
        Py_ssize_t len = PyList_Size(buffer);
        for (Py_ssize_t i = 0; i < len; ++i) {
            PyObject* item = PyList_GetItem(buffer, i);
            if (PyUnicode_Check(item)) {
                const char* line = PyUnicode_AsUTF8(item);
                printf("%s", line);
            }
        }
    }
