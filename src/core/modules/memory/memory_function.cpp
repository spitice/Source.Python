...
ReturnType CallHelper(Function func, DCCallVM* vm, unsigned long addr)
{
	ReturnType result;
	TRY_SEGV()
		result = (ReturnType) func(vm, reinterpret_cast<DCpointer>(addr));
	EXCEPT_SEGV()
	return result;
}
...
{
	TRY_SEGV()
		dcCallVoid(vm, reinterpret_cast<DCpointer>(addr));
	EXCEPT_SEGV()
}
	...
	for(int i=1; i < PyTuple_GET_SIZE(args); i++)
	{
		...
		switch(extract<DataType_t>(m_tArgs[i - 1]))
		{
			...
			case DATA_TYPE_POINTER:
			{
				...
				dcArgPointer(g_pCallVM, reinterpret_cast<DCpointer>(ulAddr));
				break;
			}
